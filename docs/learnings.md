...
## Dynamic Pool Synchronization in APIs
- **Problem**: Exposing strategy source code/metadata via `/api/strategy/{id}` that is dynamically generated by background optimization loops (Autoresearch) can lead to "Not found" errors because the memory cache (`POOL.strategies`) is loaded only at startup.
- **Solution**: Check if the requested `strategy_id` is present in memory. If not, refresh the pool from disk (`POOL.load()`) before returning. This guarantees liveness and eliminates the need to restart the FastAPI process.

## Event Delegation for Dynamic Renders
- **Problem**: Binding click event listeners directly to strategy rows in dynamic lists (winners/losers) leads to memory leaks or broken bindings when the lists are cleared and re-injected on every poll cycle.
- **Solution**: Use event delegation. Attach a single listener to the parent container (`winners-list` / `losers-list`) and read the strategy ID using `e.target.closest('.strat-row').getAttribute('data-id')`.

## SVG Sparklines for Iteration Log Trends
- **Problem**: Tabular iteration logs are hard to scan for optimization trends. Importing a heavy charting library just for small log trends is unnecessary weight.
- **Solution**: Implement lightweight inline SVG sparklines. Mapping the last 20 log run values to coordinates on an SVG polyline/path with a glassmorphism area gradient provides immediate visual feedback of optimizer convergence/plateaus.

## Interactive Parameter Control
- **Problem**: Hardcoding parameters like `pool_size` or `market_steps` inside client scripts forces code modifications for simple optimization scaling changes.
- **Solution**: Move configuration into an expandable HTML `<details>` form, and bind inputs directly into the API request payload on cycle start.

## Real-Time Multi-State Grid Filtering
- **Problem**: Dense dashboard grids (20+ strategy cards) can overwhelm operators, making it hard to track status changes (e.g. breached vs active cards).
- **Solution**: Introduce filter chips (`All`, `Profitable`, `Above Target`, `Breached`) and re-apply filters against the latest local memory snapshot. This maintains responsiveness and prevents redundant API traffic.
